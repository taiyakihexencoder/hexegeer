...
namespace hexegeer.internallib {
	...
	public partial struct FieldObservationSystem : ISystem {
		private EntityQuery _preloadQuery;
		private EntityQuery _observerQuery;
		private EntityQuery _loadFieldQuery;
		private EntityQuery _unloadFieldQuery;

		void ISystem.OnCreate(ref SystemState state) {
			_preloadQuery = new EntityQueryBuilder(Allocator.Temp)
				.WithAll<FieldObservationPoint, LocalToWorld, FieldPreload>()
				.Build(ref state);

			_observerQuery = new EntityQueryBuilder(Allocator.Temp)
				.WithAll<FieldObservationPoint, LocalToWorld>()
				.WithNone<FieldPreload>()
				.Build(ref state);
			state.RequireAnyForUpdate(_preloadQuery, _observerQuery);
		...
		void ISystem.OnUpdate(ref SystemState state) {
			...
			if (!_preloadQuery.IsEmpty) {
				NativeArray<LocalToWorld> localToWorlds = _preloadQuery.ToComponentDataArray<LocalToWorld>(Allocator.TempJob);
				...
				// preloadの場合は読込後にobservationPointを削除
				EntityCommandBuffer deleteCommandBuffer = CreateCommandBuffer(ref state);
				deleteCommandBuffer.DestroyEntity(_preloadQuery, EntityQueryCaptureMode.AtPlayback);
			}

			if (!_observerQuery.IsEmpty) {
				...
				for (int i = 0; i < localToWorlds.Length; ++i) {
					...
					if (
						...
						commandBuffer.AddComponent(
							sortKey, 
							requestEntity, 
							new FieldLoadRequest { 
								id = header.ValueRO.id, 
								keep = isPreload,
							}
						);
						break;
		...
		partial struct CheckUnloadJob : IJobEntity {
			...
			void Execute(
				...
				commandBuffer.AddComponent(sortKey, unloadContentsEntity, new ContentKeyUnloadRequest{ contentKey = header.ValueRO.contentKey, });
			}
		}
	}
