...
namespace hexegeer {
	[UpdateInGroup(typeof(HexegeerInputModuleSystemGroup))]
	public partial class InputListenerSystem : SystemBase {
		...
		override protected void OnUpdate() {
			...
			if (_input.player.start.WasPressedThisFrame()) { pressed.Add(Pressed(InputButtonEventKey.Start)); }
			if (_input.player.select.WasPressedThisFrame()) { pressed.Add(Pressed(InputButtonEventKey.Select)); }
			...
			if (_input.player.start.WasReleasedThisFrame()) { released.Add(Released(InputButtonEventKey.Start)); }
			if (_input.player.select.WasReleasedThisFrame()) { released.Add(Released(InputButtonEventKey.Select)); }
		}
